...
DataTransfer::~DataTransfer() {
    ...
    if (this->updateVacuumUrl != NULL) {
        free(this->updateVacuumUrl);
    }
    if (this->updateTemperatureUrl != NULL) {
        free(this->updateTemperatureUrl);
    }
    if (this->updateFocusUrl != NULL) {
        free(this->updateFocusUrl);
    }
    if (this->updateMountStatusUrl != NULL) {
        free(this->updateMountStatusUrl);
    }
    if (this->updateCameraStatusUrl != NULL) {
        free(this->updateCameraStatusUrl);
    }
    if (this->updateObsCtlSysStatusUrl != NULL) {
        free(this->updateObsCtlSysStatusUrl);
    }
...
void DataTransfer::initParameter(char *rootUrl) {
    ...
    joinStr(rootUrl, UPLOAD_FOCUS, &this->updateFocusUrl);
    joinStr(rootUrl, UPLOAD_MOUNT_STATUS, &this->updateMountStatusUrl);
    joinStr(rootUrl, UPLOAD_CAMERA_STATUS, &this->updateCameraStatusUrl);
    joinStr(rootUrl, UPLOAD_OBS_CTL_SYS_STATUS, &this->updateObsCtlSysStatusUrl);
...
int DataTransfer::uploadFocus(int fbfId, int focus, int cameraId, char statusstr[]) {
    ...
    return uploadDatas(this->updateFocusUrl, "", params, files, statusstr);
}
/**
 * 上传转台状态
 * @param utc : 2018-06-14T08:11:30
 */
int DataTransfer::uploadMountStatus(char *groupId, char *unitId, char *utc, int state,
    ...
    return uploadDatas(this->updateMountStatusUrl, "", params, files, statusstr);
}
 ...
 * @param utc : 2018-06-14T08:11:30
 */
int DataTransfer::uploadCameraStatus(char *groupId, char *unitId, char *camId, char *utc, int mcState,
 ...
 * @param utc : 2018-06-14T08:11:30
 */
int DataTransfer::uploadObsCtlSysStatus(char *groupId, char *unitId, char *utc, int state,
    ...
    sprintf(tstr, "%d", mountStatus);
    params.insert(std::pair<string, string>("mountStatus", tstr));
    sprintf(tstr, "%d", cameraStatus);
    params.insert(std::pair<string, string>("cameraStatus", tstr));
    return uploadDatas(this->updateObsCtlSysStatusUrl, "", params, files, statusstr);
}
 ...
 */
int DataTransfer::uploadTemperature(char *groupId, char *unitId, char *camId,
 ...
 * @param online, if is on line, set online to 1; if is off line, set online to 0;
 */
int DataTransfer::uploadVacuum(char *groupId, char *unitId, char *camId,
